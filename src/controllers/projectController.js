...
const updateInfo = asyncHandler(async (req, res) => {
  const project = await projectService.updateProjectInfo(req.user, req.params.id, req.body);
  res.status(200).json({ success: true, data: project });
});

module.exports = { list, create, getById, assign, createForClient, remove, updateDeadline, updateInfo };
